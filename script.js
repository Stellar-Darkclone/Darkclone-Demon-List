...
async function updateRankings() {
  try {
    const response = await fetch("https://api.demonlist.org/demons");
    const demons = await response.json();

    // Build lookup table: name → rank
    const demonMap = {};
    demons.forEach(d => {
      demonMap[d.name.toLowerCase()] = d.position;
    });

    // Go through all demon list items
    document.querySelectorAll("ul.clean li").forEach(li => {
      // Extract demon name after the dash
      const text = li.textContent.trim();
      const dashIndex = text.indexOf(" - ");
      if (dashIndex === -1) return;

      const rankPart = text.substring(0, dashIndex).trim();
      const restPart = text.substring(dashIndex + 3).trim();

      // Try to match demon name (before any parentheses/brackets)
      const nameMatch = restPart.match(/^([A-Za-z0-9\s']+)/);
      if (!nameMatch) return;

      const demonName = nameMatch[1].trim().toLowerCase();

      if (demonMap[demonName]) {
        // Replace old number with updated rank
        li.textContent = `${demonMap[demonName]} - ${restPart}`;
      }
    });

    console.log("Rankings updated from demonlist.org ✅");
  } catch (err) {
    console.error("Error fetching demonlist data:", err);
  }
}
